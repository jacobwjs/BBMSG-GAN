...
# Stores the dataset to train on.
# XXX:
# - Maybe move this to cloud storage bucket if it's a repeat training dataset
#
RUN mkdir /workdir/data
# Models are stored here during training.
#
RUN mkdir /workdir/models
# If any samples are generated during training, they live here.
#
...
# Install dependencies that BBMSG-GAN needs.
#
RUN pip --no-cache-dir install tensorboardX
run pip --no-cache-dir install tqdm
run pip --no-cache-dir install imageio
run pip --no-cache-dir install scipy


# Perform the actual copy of source and data.
#
COPY ./bbmsg_gan/sourcecode /workdir/sourcecode
